...
controls.addEventListener("lock", function () {
  console.log('controls.addEventListener("lock"')
  instructions.style.display = "none";
  blocker.style.display = "none";
});

controls.addEventListener("unlock", function () {
  console.log('controls.addEventListener("unlock"')
...
function addSky(scene) {
  ...
  loader.setPath("/textures/"); // Путь к текстурам
...
function updatePlayer(delta, keysPressed) {
  ...
  if (play === "run" || play === "walk") {
    ...
    playerState.model.quaternion.rotateTowards(
      playerState.rotateQuaternion,
      0.2
    );
    ...
    playerState.walkDirection.applyAxisAngle(
      playerState.rotateAngle,
      directionOffset
    );

    const velocity =
      play === "run" ? playerState.runVelocity : playerState.walkVelocity;
